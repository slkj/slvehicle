...
var DateUtil = function() {
	 ...
	 * @param date
	 *            Date日期对象, 如果缺省，则为当前时间 YYYY/yyyy/YY/yy 表示年份 MM/M 月份 W/w 星期
	 *            dd/DD/d/D 日期 hh/HH/h/H 时间 mm/m 分钟 ss/SS/s/S 秒
	...
	this.dateToStr = function(formatStr, date) {
		...
		str = str.replace(/yy|YY/, (date.getYear() % 100) > 9 ? (date.getYear() % 100).toString()
				: '0' + (date.getYear() % 100));
		str = str.replace(/MM/, date.getMonth() > 9 ? (date.getMonth() + 1) : '0'
				+ (date.getMonth() + 1));
		str = str.replace(/M/g, date.getMonth());
		str = str.replace(/w|W/g, Week[date.getDay()]);

		str = str.replace(/dd|DD/, date.getDate() > 9 ? date.getDate().toString() : '0'
				+ date.getDate());
		str = str.replace(/d|D/g, date.getDate());

		str = str.replace(/hh|HH/, date.getHours() > 9 ? date.getHours().toString() : '0'
				+ date.getHours());
		str = str.replace(/h|H/g, date.getHours());
		str = str.replace(/mm/, date.getMinutes() > 9 ? date.getMinutes().toString() : '0'
				+ date.getMinutes());
		str = str.replace(/m/g, date.getMinutes());

		str = str.replace(/ss|SS/, date.getSeconds() > 9 ? date.getSeconds().toString() : '0'
				+ date.getSeconds());
	...
	this.dateAdd = function(strInterval, num, date) {
		...
		case 'm':
			return new Date(date.getFullYear(), (date.getMonth()) + num, date.getDate(), date
					.getHours(), date.getMinutes(), date.getSeconds());
		case 'y':
			return new Date((date.getFullYear() + num), date.getMonth(), date.getDate(), date
					.getHours(), date.getMinutes(), date.getSeconds());
		}
	}
	/**
	...
	this.dateDiff = function(strInterval, dtStart, dtEnd) {
		...
		case 'm':
			return (dtEnd.getMonth() + 1) + ((dtEnd.getFullYear() - dtStart.getFullYear()) * 12)
					- (dtStart.getMonth() + 1);
		case 'y':
	...
	this.daysBetween = function(DateOne, DateTwo) {
		...
		var cha = ((Date.parse(OneMonth + '/' + OneDay + '/' + OneYear) - Date.parse(TwoMonth + '/'
	 ...
	 */
	this.DiffLong = function(datestr1, datestr2) {
	...
	this.strFormatToDate = function(formatStr, dateStr) {
		...
		var hour = 0;
		if (((start = formatStr.indexOf('HH')) > -1 || (start = formatStr.indexOf('hh')) > 1)
				&& start < len) {
